...
ggplot(results, aes(x = actual, y = predicted)) +
  ...
  labs(title = "Predicted vs Actual MEDV", x = "Actual MEDV", y = "Predicted MEDV") +
  theme_minimal()

# Scatter 3d
library(plotly)
plot_ly(data = housing.df, x = ~LSTAT, y = ~RM, z = ~MEDV, 
        type = "scatter3d", mode = "markers", 
        marker = list(size = 3, color = ~MEDV, colorscale = 'Viridis', showscale = TRUE))
  layout(title = "3D Scatter Plot of MEDV, LSTAT, and RM",
